...
import { ADD_TO_TIMEOUT } from './config.js';
...
const controlAddRecipe = async function(data){
  try{
    ...
    setTimeout(() => {
      ...
      addRecipeView.togglerFuntion();
    }, ADD_TO_TIMEOUT*1000);
    // bookmarkView.render()
    // Giving this so whenever the page reloads we can get access to the hash that we mensioned here ;
    window.history.pushState(null,'',model.state.recipe.state);
